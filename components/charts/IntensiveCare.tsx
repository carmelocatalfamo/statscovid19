...
export const IntensiveCare: FC<Props> = ({ data, size }) => {
  ...
  useEffect(() => {
    try {
      const timeRange = window.localStorage.getItem('timeRangeIntensiveCare')
  ...
  useEffect(() => {
    window.localStorage.setItem('timeRangeIntensiveCare', JSON.stringify(timeRange))
  }, [timeRange])
  ...
  return (
    <Card
      ...
      title={() => (
        <Header>
          <Title>Ricoverati in terapia intensiva</Title>
          <TimeRangeSelect
            instanceId='intensive_care'
            value={timeRange}

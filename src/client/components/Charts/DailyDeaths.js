...
export default withTheme(({ theme, regionCode }) => {
  ...
  const data = dataPerDay
    .map((day, index, list) => {
      return {
        date: day.data,
        deaths: !index ? day.deceduti : (day.deceduti - list[index - 1].deceduti)
      }
    })
    .filter(({ date }) => (
...
const getDescriptionByKey = key => {
  switch (key) {
    case 'deaths':
      return ': totale deceduti del giorno corrente meno totale deceduti del giorno precedente.'
